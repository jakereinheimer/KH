...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  iprob                  = pin->GetOrAddInteger("problem","iprob",1); //problem identifier, since KH file has 3 separate problems that can be run
  gamma_adi              = pin->GetReal("hydro",   "gamma"); //adiabatic gamma factor
  rho_0                  = pin->GetReal("problem", "rho_0"); //density of gas on the sides
  pgas_0                 = pin->GetReal("problem", "pgas_0"); //pressure of gas on the sides
  density_contrast       = pin->GetReal("problem", "density_contrast"); //fraction between the density in the middle and the density on the side
  vel_shear              = pin->GetReal("problem", "vel_shear"); //relative shear velocity?
  ...
  smoothing_thickness = pin->GetReal("problem", "smoothing_thickness"); //initialize a gradual change in density at the boundary. smoothing_thickness is the width of the boundary
  smoothing_thickness_vel = pin->GetOrAddReal("problem", "smoothing_thickness_vel", -100.0); //velocity of the boundary layer
  if (smoothing_thickness_vel < 0){
    smoothing_thickness_vel = smoothing_thickness;
  } //why? This seems random
  vel_pert       = pin->GetReal("problem", "vel_pert"); //velocity of the perturbation 
  lambda_pert         = pin->GetReal("problem", "lambda_pert"); //length of the boundary layer?
  z_top               = pin->GetReal("problem", "z_top"); //boundary of the cold gas layer
  z_bot               = pin->GetReal("problem", "z_bot"); //''

  // Boundary Conditions -----------------------------------------------------------------
  bool ConstantShearInflowOuterX2_on = pin->GetOrAddBoolean("problem", "ConstantShearInflowOuterX2_on", false); //indicate if shear is in the 2nd or 3rd axis, probably always want to be true
  ...
  if(mesh_bcs[BoundaryFace::outer_x3] == GetBoundaryFlag("user")) {
    if (ConstantShearInflowOuterX3_on) EnrollUserBoundaryFunction(BoundaryFace::outer_x3, ConstantShearInflowOuterX3); //MODIFICATION: added "BoundaryFace::" before "inner_x3" amd "outer_x3"
  }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  bool noisy_IC = pin->GetOrAddBoolean("problem", "noisy_IC", false); //noisy initial condition? Like initializing a random-ish field??
  std::int64_t iseed = -1 - gid; //what does gid do?
  ...
  if (iprob == 1) {
    ...
    for (int k=kl; k<=ku; k++) {
      ...
      for (int j=jl; j<=ju; j++) {
        ...
        for (int i=il; i<=iu; i++) {
          Real x = pcoord->x1v(i);
          Real r = std::sqrt(SQR(z) + SQR(x)); //set up radial step function for a cylinder, r = sqrt(z^2 + y^2)
          // 3D
          if (block_size.nx3 > 1) {
            phydro->u(IDN,k,j,i) = 0.5 * (density_contrast + (rho_0 * (1.0 + (1.0 - density_contrast) * (std::tanh((r-z_top)/smoothing_thickness) ) ) ) );
            phydro->u(IPR,k,j,i) = pgas_0;
            phydro->u(IVY,k,j,i) = 0.5 * vel_shear * std::tanh((z_top-r)/smoothing_thickness_vel);
            phydro->u(IVX,k,j,i) = 0.0;
            phydro->u(IVZ,k,j,i) = 0.0;
            // Perturbations
            if (lambda_pert > 0.0){
              phydro->u(IVZ,k,j,i) = vel_pert;
              phydro->u(IVZ,k,j,i) *= (std::exp(-SQR((r-z_bot)/smoothing_thickness_vel)) + std::exp(-SQR((r-z_top)/smoothing_thickness_vel) ) );
              phydro->u(IVZ,k,j,i) *= std::sin(2*PI*x/lambda_pert) * std::sin(2*PI*z/lambda_pert) ;

              phydro->u(IVX,k,j,i) = vel_pert;
              phydro->u(IVX,k,j,i) *= (std::exp(-SQR((r-z_bot)/smoothing_thickness_vel)) + std::exp(-SQR((r-z_top)/smoothing_thickness_vel) ) );
              phydro->u(IVX,k,j,i) *= std::sin(2*PI*x/lambda_pert) * std::sin(2*PI*y/lambda_pert) ;
            }
          }
          // 2D
            ...
            if (noisy_IC){
              phydro->u(IM2,k,j,i) *= ran2(&iseed); //if the noisy IC is declared, introduce some random motion in the y direction (for 2D, no 3D implementation written yet)
            }
...
void ConstantShearInflowInnerX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                    FaceField &b, Real time, Real dt,
                    int il, int iu, int jl, int ju, int kl, int ku, int ngh) {
  // copy hydro variables into ghost zones on left side of y (x2)
    for (int k=kl; k<=ku; k++) {
    //goes through z dimension from low to upper lim
    for (int j=1; j<=ngh; j++) {
      //goes through each ghost zone in the x,z plane at y=0 basically (the left side)
      for (int i=il; i<=iu; i++) {
        //goes through x dimension from low to upper lim
        for (int n=0; n<(NHYDRO); n++) {
        //goes through each kind of hydro variable, like IDN or IVY
        //Real y = pco->x2v(jl-j);
        Real x = pco->x1v(il-i);
        // defines a variable x that has the x coordinate of the i variable based on the place in loop
        Real z = pco->x3v(kl-k);
        // defines a variable z that has the z coordinate of the k variable based on the place in loop
        Real r = std::sqrt(SQR(z) + SQR(x));
        // defines a value r which is the radial distance based on x and z, which is just the distance from center of point of xz plane
        prim(n,k,jl-j,i) = prim(n,k,jl,i);
        //I am not really sure what this does
        //does it set all nhydros to the initial value, to then be changed afterwards?
        if ( n == IPR ){
          prim(IPR,k,jl-j,i) = pgas_0;
          //sets pressure to pgas_0, which is a user input
        } 
        if ( n == IDN ){
          //prim(IDN,k,jl-j,i) = rho_0 * (1.0 + (density_contrast-1.0) * 0.5 * ( std::tanh((r-z_bot)/smoothing_thickness) ));
          prim(IDN,k,jl-j,i) = 0.5 * (density_contrast + (rho_0 * (1.0 + (1.0 - density_contrast) * (std::tanh((r-z_top)/smoothing_thickness) ) ) ) );
          //sets the density at this point to the result of our equation, which creates a cylinder
        } 
        if ( n == IVY ){
          //prim(IVY,k,jl-j,i) = vel_shear * ( 0.5 - 0.5 * ( std::tanh((r-z_bot)/smoothing_thickness) ));
          prim(IVY,k,jl-j,i) = 0.5 * vel_shear * std::tanh((z_top-r)/smoothing_thickness_vel);
          //sets the y velocity 
        } 
      }
    }}
...
#pragma omp simd
      for (int i=il; i<=iu; i++) {
        ...
        if ( n == IDN ){
          prim(IDN,k,ju+j,i) = rho_0 * (1.0 + (density_contrast-1.0) * 0.5 * ( std::tanh((r-z_bot)/smoothing_thickness) ) );
        } 
        if ( n == IVX ){
          prim(IVX,k,ju+j,i) = vel_shear * ( 0.5 - 0.5 * ( std::tanh((r-z_bot)/smoothing_thickness) ));
        } 
      }
    }}
  }
  ...
  for (int n=0; n<(NHYDRO); n++) {
    ...
    for (int j=jl; j<=ju; j++) {
      for (int i=il; i<=iu; i++) {
        ...
        if ( n == IDN ){
          prim(IDN,kl-k,j,i) = rho_0 * (1.0 + (density_contrast-1.0) * 0.5 * ( std::tanh((r-z_bot)/smoothing_thickness) - std::tanh((r-z_top)/smoothing_thickness) ) );
        } 
        if ( n == IVX ){
          prim(IVX,kl-k,j,i) = vel_shear * ( 0.5 - 0.5 * ( std::tanh((r-z_bot)/smoothing_thickness) - std::tanh((r-z_top)/smoothing_thickness) ));
        } 
      }
    }}
  }
  ...
  for (int n=0; n<(NHYDRO); n++) {
    ...
    for (int j=jl; j<=ju; j++) {
      for (int i=il; i<=iu; i++) {
        ...
        if ( n == IDN ){
          prim(IDN,ku+k,j,i) = rho_0 * (1.0 + (density_contrast-1.0) * 0.5 * ( std::tanh((r-z_bot)/smoothing_thickness) - std::tanh((r-z_top)/smoothing_thickness) ) );
        } 
        if ( n == IVX ){
          prim(IVX,ku+k,j,i) = vel_shear * ( 0.5 - 0.5 * ( std::tanh((r-z_bot)/smoothing_thickness) - std::tanh((r-z_top)/smoothing_thickness) ));
        } 
      }
    }}
  }
